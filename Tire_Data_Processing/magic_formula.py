...
def mf61_Fy(x, p, scaling_factors=None, coordinate_system = 'adapted SAE'):
    ...
    mu_y = (p_dy1 + p_dy2*dfz) * (1 + p_dy3*dpi + p_py4*dpi**2) * (1 - p_dy3*gamma**2) * lam_muy  # 4.E23
    D_y = mu_y * F_z  # 4.E22
    
    K_yalpha = p_ky1* fz0pr* (1 + p_py1*dpi)* (1-p_ky3*np.abs(gamma)) * np.sin( p_ky4* np.arctan(F_z / (fz0pr*(p_ky2 + p_ky5*gamma**2)*(1 + p_py2*dpi))) )  * lam_kyalpha  # 4.E25
    ...
    S_vygamma = F_z * (p_vy3 + p_vy4*dfz) * gamma * lam_kygamma*lam_muy # E4.28
    S_vy = F_z*(p_vy1 + p_vy2*dfz)*lam_vy*lam_muy  + S_vygamma# 4.E29
    K_ygamma0 = F_z*(p_ky6 + p_ky7*dfz) * (1 + p_py5*dpi) * lam_kygamma# 4.E30

    S_hy = (p_hy1 + p_hy2*dfz) * lam_hy + (K_ygamma0*gamma-S_vygamma)/K_yalpha  # 4.E27
    alpha_y = alpha + S_hy  # 4.E20

    E_y = (p_ey1 + p_ey2*dfz) * (1 + p_ey5*gamma**2 - (p_ey3 + p_ey4*gamma)*np.sign(alpha_y)) * lam_ey  # 4.E24 (<=1)
...
def mf61_Mz(x, p, scaling_factors=None):
    """
    Self-Aligning torque from pure side slip MF6.1 originally developed by TNO
    The equations below come from Tire and Vehicle Dynamics by Hans Pacejka 
    """
    #Model Parameters
    q_bz1, q_bz2, q_bz3, q_bz4, q_bz5, q_cz1, q_dz1, q_dz2, q_pz1, q_hz1, q_hz2, q_hz3, q_hz4= p
    ...
    C_t = np.abs(q_cz1) # 4.E41 NOTE abs might not be necessary 
