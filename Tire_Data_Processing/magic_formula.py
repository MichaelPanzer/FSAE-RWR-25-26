...
def mf61_Fy(x, p, scaling_factors=None, coordinate_system = 'adapted SAE'):
    ...
    K_yalpha = p_ky1* f_z0* (1 + p_py1*dpi)* (1-p_ky3*np.abs(gamma)) * np.sin( p_ky4* np.arctan( F_z / (f_z0*(p_ky2+p_ky5*gamma**2)*(1+p_py2*dpi)) ) )  * lam_kyalpha  # 4.E25
    ...
    S_vygamma = F_z * (p_vy3 + p_vy4*dfz) * gamma * lam_kygamma*lam_muy
    S_vy = F_z * (p_ky6 + p_ky7*dfz) * (1+p_py5*dpi) + lam_kygamma # 4.E29
    K_ygamma0 = F_z*(p_ky6 + p_ky7*dfz) * (1 + p_py5*dpi) * lam_kygamma# 4.E30

    S_hy = (p_hy1 + p_hy2*dfz) * lam_hy + (K_ygamma0*gamma-S_vygamma)/(K_yalpha) -1  # 4.E27
